...
class LinkerStudio:
    ...
    def start_linking(self):
        ...
        candidate_files = [
            ...
            and os.path.join(search_dir, f) not in final_list
        ]
        ...
        while True:
            ...
            for cand_path in list(candidate_files):
                ...
                if cand_defines & missing:
                    self.log(
                        f"[AUTO-FOUND] Adding '{os.path.basename(cand_path)}' to satisfy requirements.",
                        "green"
                    )
                    ...
                    self.file_listbox.insert(
                        tk.END,
                        f" {os.path.basename(cand_path)} (Auto-Added)"
                    )
            ...
            if not found_any:
                self.log(
                    f"CRITICAL: Could not find providers for: {list(missing)}",
                    "error"
                )

                messagebox.showerror(
                    "Error",
                    f"Missing symbols could not be resolved: {missing}"
                )
                return
        ...
        try:
            ...
            # ÇOK ÖNEMLİ: relative path'i absolute path'e çevir
            e_p = os.path.abspath(e_p)
            m_p = os.path.abspath(m_p)
            h_p = os.path.abspath(h_p)

            self.log("\n--- SMART LINKING SUCCESSFUL ---", "green")
            self.log(f"ESTAB dosyası: {e_p}", "cyan")
            self.log(f"MEM dosyası: {m_p}", "cyan")

            self.log("\nESTAB Preview:", "cyan")

            with open(e_p, "r", encoding="utf-8", errors="ignore") as ef:
                self.log(ef.read())

            messagebox.showinfo(
                "Success",
                "Linking complete with auto-resolved dependencies!"
            )
            ...
            messagebox.showerror(
                "Dosya Kilitli",
                "output.mem veya ESTAB.txt yazılamıyor.\n\n"
                "Açıksa kapat, sonra tekrar dene."
            )
